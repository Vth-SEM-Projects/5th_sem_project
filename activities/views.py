    ...
    if request.user.id == None:
        ...
        for l in li:
            if l['job_id'] in jobs or l['last_date_apply'] < date.today():
                ...
                current.append(l)
        return render(request,'vacancy_table.html',{'vacancies':zip(current,companies),'value':value,'is_admin':is_admin});
...
def eligibility_role(request):
    if request.method=='POST':
        role_check = request.POST.get('role','')
        li = Vacancies.objects.values('job_id','role','link','package','last_date_apply','location','no_of_vacancies','type_of_branch','no_of_applications','company_id')
        jobs = []
        current = []
        companies = []
        for no in li:
            jobs.append(no['job_id'])
        for l in li:
            if l['role'] == role_check:
                companies.append(Company.objects.filter(id=l['company_id']).values('name').first())
                current.append(l)
                print(l['job_id'])
        return render(request,'temp.html',{'vacancies':zip(current,companies)});
    else:
        return HttpResponseRedirect("/eligibility_form")

def eligibility_package(request):
    if request.method=='POST':
        min_check = request.POST.get('pack1','')
        max_check = request.POST.get('pack2','')
        li = Vacancies.objects.values('job_id','role','link','package','last_date_apply','location','no_of_vacancies','type_of_branch','no_of_applications','company_id')
        jobs = []
        current = []
        companies = []
        for l in li:
            a = l['package']
            if min_check <= l['package'] <= max_check:
                companies.append(Company.objects.filter(id=l['company_id']).values('name').first())
                current.append(l)
                print(l['job_id'])
        return render(request,'temp.html',{'vacancies':zip(current,companies)});
...
def eligibility_location(request):
    if request.method=='POST':
        location = request.POST.get('location','')
        li = Vacancies.objects.values('job_id','role','link','package','last_date_apply','location','no_of_vacancies','type_of_branch','no_of_applications','company_id')
        jobs = []
...
def eligibility_branch(request):
    if request.method=='POST':
        ...
        companies = []
        for l in li:
            if l['type_of_branch']==branch or l['type_of_branch']=="all":
                companies.append(Company.objects.filter(id=l['company_id']).values('name').first())
                current.append(l)
                print(l['job_id'])
        return render(request,'temp.html',{'vacancies':zip(current,companies)});
